// Heaps
// Implement the array-based representation of binary heap (in this case a min-heap)
...
void heap_insert (BinHeap* h, int value){
  h->size = h->size + 1;
  h->H[h->size-1] = INT_MAX;
  heap_decrease_key(h, (h->size-1), value);
}
...
int main(){
  ...
  remove_minimum(&myheap);
  
  heap_print(&myheap);

  heap_decrease_key(&myheap, 3, 1);
