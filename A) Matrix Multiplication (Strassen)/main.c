...
#include "matrix.h"
#include "strassen.h"
#include "strassen_improved1.h"
#include "strassen_improved2.h"
...
int main(int argc, char *argv[]) {
  ...
  printf("\nExecution time with Naive / Strassen / Strassen_imp1 / Strassen_imp2, for different matrix sizes:\n\n");
  ...
  for (size_t i=1; i<=n; i*=2) {
    ...
    clock_gettime(CLOCK_REALTIME, &b_time);
    improved1_strassen(C2, A, B, i);
    clock_gettime(CLOCK_REALTIME, &e_time);

    printf("\t%lf", get_execution_time(b_time, e_time));

    //printf("\t%d\t", same_matrix(C0, i, i, C2, i, i));

    clock_gettime(CLOCK_REALTIME, &b_time);
    improved2_strassen(C3, A, B, i);
    clock_gettime(CLOCK_REALTIME, &e_time);

    printf("\t%lf", get_execution_time(b_time, e_time));

    //printf("\t%d\t", same_matrix(C0, i, i, C3, i, i));*/
